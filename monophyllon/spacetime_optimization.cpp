...
void Spacetime::simplex_solver()
{
  ...
  do {
    ...
    geometry->compute_squared_distances();
    for(i=0; i<nv; ++i) {
      if (skeleton->active_event(i)) skeleton->events[i].set_geometry_modified(true);
    }
    compute_volume();
    compute_obliquity();
    structural_deficiency();
    ...
    if (f < fitness[0].second) {
      ...
      if (error < f) {
        ...
        for(i=1; i<=system_size; ++i) {
          ...
          geometry->compute_squared_distances();
          for(j=0; j<nv; ++j) {
            if (skeleton->active_event(j)) skeleton->events[j].set_geometry_modified(true);
          }
          compute_volume();
          compute_obliquity();
          structural_deficiency();
...
void Spacetime::optimize()
{
  ...
  if (solver == Geometry_Solver::minimal) {
    int its = 0;
    bool found;
    std::set<int> modified_vertices,candidates,bbarrel;
    std::set<int>::const_iterator it;
    ...
    for(i=0; i<nv; ++i) {
      ...
      bbarrel.insert(i);
      // Now check to see if all of this event's neighbours have a geometric deficiency > 0
      found = false;
