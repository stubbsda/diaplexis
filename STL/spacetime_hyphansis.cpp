...
bool Spacetime::stellar_addition(int base,int sheet)
{
  ...
  for(it=events[base].entourage.begin(); it!=events[base].entourage.end(); ++it) {
    if (simplices[1][*it].ubiquity[sheet] == 1) {
...
bool Spacetime::stellar_deletion(int base,int sheet)
{
  ...
  for(i=0; i<m; ++i) {
    if (simplices[2][i].ubiquity[sheet] == 0) continue;
...
int Spacetime::compression(double threshold,std::set<int>& vmodified)
{
  ...
  std::set<int>::const_iterator it;
  double s,alpha;
  std::vector<int> candidates,locus;
  const int nv = (signed) events.size();
  const int ne = (signed) simplices[1].size();
  const int nt = (signed) codex.size();
...
bool Spacetime::expansion(int base,int sheet)
{
  ...
  const int nt = (signed) codex.size();
  const int ne = (signed) simplices[1].size();
  const double tau = std::abs(events[base].deficiency) + 25.0;
  ...
  for(it=N.begin(); it!=N.end(); ++it) {
    ...
    s1.initialize(m,n,locus);
    qt = index_table[1].find(s1.vertices);
    if (qt == index_table[1].end()) {
      simplices[1].push_back(s1);
      index_table[1][s1.vertices] = simplices[1].size() - 1;
    }
    else {
      simplices[1][qt->second].ubiquity[sheet] = 1;
    }
...
void Spacetime::vertex_fusion(int n1,int n2,int sheet)
{
  ...
  if (sheet == -1) {
    ...
    locus = events[n2].ubiquity;
    for(j=0; j<nt; ++j) {
      events[n2].ubiquity[j] = 0;
      events[n1].ubiquity[j] |= locus[j];
    }
    delete[] mutation;
  ...
  do {
    ...
    for(i=0; i<nc; ++i) {
      ...
      for(it=vx.neighbours.begin(); it!=vx.neighbours.end(); ++it) {
        ...
        for(k=0; k<nt; ++k) {
          if (RND.drandom() > 0.33) locus[k] = 1;
        }
        if (ghost(locus)) locus[RND.irandom(nt)] = 1;
