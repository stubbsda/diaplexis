...
double Spacetime::total_energy(int sheet) const
{
  ...
  if (sheet == -1) {
    for(int i=0; i<nv; ++i) {
      if (!ghost(events[i].ubiquity)) sum += events[i].get_energy();
    }
  }
  else {
    for(int i=0; i<nv; ++i) {
      if (events[i].ubiquity[sheet] == 1) sum += events[i].get_energy();
    }
  }
  return sum;
}
...
{  
  ...
  for(i=3; i<=Spacetime::ND; ++i) {
    ...
    for(j=0; j<n; ++j) {
      if (ghost(simplices[i][j].ubiquity) || !simplices[i][j].modified) continue;
...
void Spacetime::compute_lengths()
{
  ...
  for(i=0; i<ne; ++i) {
    if (ghost(simplices[1][i].ubiquity) || !simplices[1][i].modified) continue;
    simplices[1][i].get_vertices(vx);
