...
void Spacetime::get_ubiquity(int d,int n,std::string& output) const
{
  ...
  if (d == 0) {
    ...
    if (simplices[d][n].ubiquity[nt-1] == 1) {
      output += "1}";
    }
    else {
      output += "0}";
...
{
  if (d == 0) {
    return ghost(events[n].ubiquity);
  }
  else {
    return ghost(simplices[d][n].ubiquity);
  }
}
  ...
  if (sheet == -1) {
    for(i=0; i<nbase; ++i) {
      do {
        ...
        if (ghost(events[v].ubiquity)) continue;
  ...
  if (sheet == -1) {
    for(i=0; i<nvertex; ++i) {
      ...
      for(it=events[i].entourage.begin(); it!=events[i].entourage.end(); it++) {
        if (!ghost(simplices[1][*it].ubiquity)) v++;
      }
      sum += v;
      if (v > mx) mx = v;
      if (v < mn) mn = v;
    ...
    for(i=0; i<nvertex; ++i) {
      ...
      v = 0;
      for(it=events[i].entourage.begin(); it!=events[i].entourage.end(); it++) {
        if (simplices[1][*it].ubiquity[sheet] == 1) v++;
      }
      sum += v;
      if (v > mx) mx = v;
      if (v < mn) mn = v;
...
int Spacetime::chromatic_number(int sheet) const
{
  ...
  for(i=0; i<nvertex; ++i) {
    ...
    if (events[i].ubiquity[sheet] == 0) skip[i] = true;
  }
  for(i=0; i<nvertex; ++i) {
    pcolour.push_back(-1);
  }
...
int Spacetime::entourage(int v,int sheet) const
{
...
double Spacetime::return_probability(int v,int length,int sheet) const
{
    ...
    for(i=0; i<ntrials; ++i) {
      ...
      for(j=0; j<length; ++j) {
        do {
          ...
          if (simplices[1][qt->second].ubiquity[sheet] == 1) break;
        } while(true);
        current = next;
        if (current == v) {
          home = true;
...
int Spacetime::spanning_tree(std::vector<int>& tree_edges,int sheet) const
{
  ...
  if (sheet == -1) {
    ...
    m = 0;
    for(n=0; n<(signed) events.size(); ++n) {
      if (!ghost(events[n].ubiquity)) m++;
    }
  }
  else {
    for(n=0; n<(signed) events.size(); ++n) {
      if (events[n].ubiquity[sheet] == 1) {
        current.insert(n);
        break;
      }
    }
    ...
    m = 0;
    for(n=0; n<(signed) events.size(); ++n) {
      if (events[n].ubiquity[sheet] == 1) m++;
    }
  }
...
{
  bool output = (simplices[d][i].ubiquity[sheet] == 1) ? true : false;
  return output;
}
...
int Spacetime::dimension(int sheet) const
{
  ...
  if (sheet == -1) {
    for(i=ND; i>0; i--) {
      for(j=0; j<(signed) simplices[i].size(); ++j) {
        if (!ghost(simplices[i][j].ubiquity)) return i;
      }
    }
    for(i=0; i<(signed) events.size(); ++i) {
      if (!ghost(events[i].ubiquity)) return 0;
    }
  }
  else {
    for(i=ND; i>0; i--) {
      for(j=0; j<(signed) simplices[i].size(); ++j) {
        if (simplices[i][j].ubiquity[sheet] == 1) return i;
      }
    }
    for(i=0; i<(signed) events.size(); ++i) {
      if (events[i].ubiquity[sheet] == 1) return 0;
    }
  }
  return -1;
}
...
int Spacetime::total_dimension(int sheet) const
{
  ...
  if (sheet == -1) {
    ...
    for(i=0; i<(signed) events.size(); ++i) {
      if (events[i].ubiquity[sheet] == 1) sum += vertex_dimension(i,sheet);
    }
  }
  return sum;
}
...
int Spacetime::structural_index(int sheet) const
{
  ...
  if (sheet == -1) {
    ...
    for(i=0; i<(signed) events.size(); ++i) {
      if (events[i].ubiquity[sheet] == 1) sum++;
    }
    for(i=1; i<d; ++i) {
      l = 0;
      n = (signed) simplices[i].size();
      for(j=0; j<n; ++j) {
        if (simplices[i][j].ubiquity[sheet] == 1) l++;
      }
      sum += (1+i)*l;
    }
  }
...
int Spacetime::vertex_valence(int v,int sheet) const
{
  ...
  for(it=events[v].entourage.begin(); it!=events[v].entourage.end(); it++) {
    if (simplices[1][*it].ubiquity[sheet] == 1) nd++;
  }
  return nd;
}
...
int Spacetime::vertex_dimension(int v,int sheet) const
{
  ...
  if (sheet == -1) {
    ...
    if (!ghost(events[v].ubiquity)) return 0;
  }
  else {
    for(i=ND; i>=1; i--) {
      for(j=0; j<(signed) simplices[i].size(); ++j) {
    ...
    if (events[v].ubiquity[sheet] == 1) return 0;
  }
  return -1;
}
...
bool Spacetime::consistent(int sheet) const
{
  ...
  if (sheet == -1) {
    for(i=ND; i>=2; i--) {
      ...
      for(j=0; j<n; ++j) {
        ...
        for(it=simplices[i][j].entourage.begin(); it!=simplices[i][j].entourage.end(); it++) {
          if (ghost(simplices[i+1][*it].ubiquity)) {
    ...
    for(i=0; i<(signed) simplices[1].size(); ++i) {
      ...
      if (ghost(events[vx[0]].ubiquity)) {
        std::cout << "Inactive vertex: " << vx[0] << std::endl;
        return false;
      }
      if (ghost(events[vx[1]].ubiquity)) {
    ...
    for(i=0; i<nv; ++i) {
      if (ghost(events[i].ubiquity)) continue;
      for(it=events[i].entourage.begin(); it!=events[i].entourage.end(); it++) {
        if (ghost(simplices[1][*it].ubiquity)) {
...
bool Spacetime::connected(int sheet) const
{
  ...
  if (sheet == -1) {
    ...
    do {
      for(it=change.begin(); it!=change.end(); it++) {
        ...
        for(jt=events[i].neighbours.begin(); jt!=events[i].neighbours.end(); jt++) {
          ...
          if (!ghost(simplices[1][qt->second].ubiquity)) nchange.insert(n);
        }
      }
    ...
    for(i=0; i<nvertex; ++i) {
      if (ubiquity[i] == 0 && !ghost(events[i].ubiquity)) {
        output = false;
        break;
      }
    }
    ...
    do {
      for(it=change.begin(); it!=change.end(); it++) {
        ...
        for(jt=events[i].neighbours.begin(); jt!=events[i].neighbours.end(); jt++) {
          ...
          if (ubiquity[n] == 1) continue;
          qt = index_table[1].find(make_key(i,n));
          if (simplices[1][qt->second].ubiquity[sheet] == 1) nchange.insert(n);
        }
      }
      if (nchange.empty()) break;
      for(it=nchange.begin(); it!=nchange.end(); it++) {
    ...
    for(i=0; i<nvertex; ++i) {
      if (ubiquity[i] == 0 && events[i].ubiquity[sheet] == 1) {
        output = false;
        break;
      }
    }
...
int Spacetime::component_analysis(std::vector<int>& component,int sheet) const
{
  ...
  if (sheet == -1) {
    do {
      start = -1;
      for(i=0; i<nvertex; ++i) {
        if (component[i] == -1 && !ghost(events[i].ubiquity)) {
          start = i;
          break;
        }
      }
      ...
      do {
        ...
        for(it=change.begin(); it!=change.end(); it++) {
          ...
          for(jt=events[n].neighbours.begin(); jt!=events[n].neighbours.end(); jt++) {
            m = *jt;
            qt = index_table[1].find(make_key(n,m));
            if (component[m] < 0 && !ghost(simplices[1][qt->second].ubiquity)) nchange.insert(m);
          }
        }
        if (nchange.empty()) break;
        change = nchange;
    ...
    do {
      start = -1;
      for(i=0; i<nvertex; ++i) {
        if (component[i] == -1 && events[i].ubiquity[sheet] == 1) {
          start = i;
          break;
        }
      }
      ...
      do {
        ...
        for(it=change.begin(); it!=change.end(); it++) {
          ...
          for(jt=events[n].neighbours.begin(); jt!=events[n].neighbours.end(); jt++) {
            m = *jt;
            qt = index_table[1].find(make_key(n,m));
            if (component[m] < 0 && simplices[1][qt->second].ubiquity[sheet] == 1) nchange.insert(m);
          }
        }
        if (nchange.empty()) break;
        change = nchange;
